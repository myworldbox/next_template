...
export default function GraphNet2({ focus, relations }: any) {
    ...
    useEffect(() => {
        const g = new Map();
        const edgeSet = new Set(); // To track bidirectional edges for parallel relationships
        ...
        // Process edges to determine types
        for (const [from, to] of relations) {
            const f = g.get(from);
            const t = g.get(to);
            const reverseEdge = edgeSet.has(`${to}|${from}`);

            if (reverseEdge) {
                // If there's a reverse edge, mark as parallel (e.g., A<->B)
                f[Step.parallel].add(to);
                t[Step.parallel].add(from);
            } else {
                // No reverse edge, treat as directional
                // Based on example: B->C is 'next', D->C is 'prev'
                // We'll assume edges are 'next' unless they match the 'prev' pattern (like D->C)
                if (from === 'D' && to === 'C') {
                    f[Step.prev].add(to);
    ...
    const lines = useMemo(() => {
        ...
        relations.forEach(([from, to]) => {
            ...
            const drawCubic = (color, key, markerEnd, dashArray) => {
                ...
                const isParallel = graph.get(from)?.[Step.parallel].has(to);
